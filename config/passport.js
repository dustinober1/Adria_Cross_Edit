...
module.exports = function configurePassport(app, pool) {
    ...
    // Apple Strategy
    const hasAppleConfig = process.env.APPLE_CLIENT_ID && process.env.APPLE_TEAM_ID && process.env.APPLE_KEY_ID && process.env.APPLE_PRIVATE_KEY_PATH;
    
    if (hasAppleConfig) {
        try {
            const keyPath = path.resolve(process.env.APPLE_PRIVATE_KEY_PATH);
            if (!fs.existsSync(keyPath)) {
                logger.error(`Passport: Apple Strategy skipped. Key file not found at: ${keyPath}`);
            } else {
            ...
            logger.error('Passport: Apple Strategy initialization failed:', err);
        }
    } else {
        const missing = [];
        if (!process.env.APPLE_CLIENT_ID) missing.push('APPLE_CLIENT_ID');
        if (!process.env.APPLE_TEAM_ID) missing.push('APPLE_TEAM_ID');
        if (!process.env.APPLE_KEY_ID) missing.push('APPLE_KEY_ID');
        if (!process.env.APPLE_PRIVATE_KEY_PATH) missing.push('APPLE_PRIVATE_KEY_PATH');
        
        if (missing.length > 0 && missing.length < 4) {
             logger.warn(`Passport: Apple Strategy partially configured. Missing: ${missing.join(', ')}`);
        }
    }
